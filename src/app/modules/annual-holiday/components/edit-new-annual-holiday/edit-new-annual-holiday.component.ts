...
export class EditNewAnnualHolidayComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.annualHolidayService.holidayList.subscribe((res) => {
      this.holidayList = res;
      if (localStorage.getItem('holidayList')) {
        this.holidayFixedLength = JSON.parse(
          localStorage.getItem('holidayList')
        ).length;
      }
    });
    this.annualHolidayService.openModel.subscribe((res) => {
      this.openModel = res;
    });
    ...
    this.route.paramMap.subscribe((param) => {
      ...
      if (this.urlParameter) {
        this.annualHolidayService
          .getAnnualHolidayByID(Number(this.urlParameter))
          .subscribe((res) => {
      ...
      breadCrump: [
        {
          label: this.translate.instant(
            'AnnualHoliday.List Of Annual Holidays'
          ),
          routerLink:
            '/educational-settings/annual-holiday/annual-holiday-list',
          ...
          label:
            this.urlParameter == null || this.urlParameter == ''
              ? this.translate.instant(
                  'AnnualHoliday.Define Annual Holidays Calendar'
                )
              : this.translate.instant(
                  'AnnualHoliday.Edit Annual Holidays Calendar'
                ),
          routerLink:
            this.urlParameter == null || this.urlParameter == ''
              ? '/educational-settings/annual-holiday/new-holiday'
  ...
  getScoolsIds(schools) {
    this.schoolsAddedList = [];
    schools?.forEach((element) => {
      this.schoolsAddedList.push(element.id);
    });
    return this.schoolsAddedList;
  }
  ...
  saveHoliday() {
    ...
    this.annualHolidayObj = {
      annualCalendar: {
        ar: this.annualHolidayFormGrp.value.arabicSmester,
        en: this.annualHolidayFormGrp.value.englishSmester,
      },
      year: this.annualHolidayFormGrp.value.year,
      holidayModels: this.holidayList.map((holiday) => {
        return {
          id: holiday.id,
          ...
          schoolIds: this.getScoolsIds(holiday.schools),
        };
      }),
    };
    if (this.urlParameter) {
      this.annualHolidayService
        .updateAnnualHoliday(Number(this.urlParameter), this.annualHolidayObj)
        .subscribe(
          (res) => {
            ...
            if (res.statusCode == 'OK') {
              this.toastService.success(
                this.translate.instant(
                  'AnnualHoliday.Holiday edited Successfully'
                )
              );
      ...
      this.annualHolidayService
        .addAnnualHoliday(this.annualHolidayObj)
        .subscribe(
          (res) => {
            this.isBtnLoading = false;
            if (res.statusCode == 'OK') {
              this.toastService.success(
                this.translate.instant(
                  'AnnualHoliday.Holiday added Successfully'
                )
              );
              this.router.navigate([
                '/educational-settings/annual-holiday/annual-holiday-list',
    ...
    this.annualHolidayService.holiday.subscribe((res) => {
      ...
      if (this.availableAdded == 1) {
        if (this.holidayAdded.index == null || undefined) {
          ...
          this.holidayList = this.holidayList.map((holiday, i) => {
            return {
              id: holiday?.id ? holiday?.id : 0,
              index: i + 1,
              name: { ar: holiday.name.ar, en: holiday.name.en },
              userName: {
                ar: holiday?.userName?.ar,
                en: holiday?.userName?.en,
              },
    ...
    if (this.holidayAdded.index) {
      this.holidayList.forEach((holiday) => {
        if (holiday.index == this.holidayAdded.index) {
          this.holidayList[this.holidayAdded.index - 1].id =
            this.holidayAdded.id;
          this.holidayList[this.holidayAdded.index - 1].index =
            this.holidayAdded.index;
          this.holidayList[this.holidayAdded.index - 1].name.ar =
            this.holidayAdded.name.ar;
          this.holidayList[this.holidayAdded.index - 1].name.en =
            this.holidayAdded.name.en;
          this.holidayList[this.holidayAdded.index - 1].dateFrom =
            this.holidayAdded.dateFrom;
          this.holidayList[this.holidayAdded.index - 1].dateTo =
            this.holidayAdded.dateTo;
          this.holidayList[this.holidayAdded.index - 1].schools =
            this.holidayAdded.schools;
          this.holidayList[this.holidayAdded.index - 1].flexibilityStatus =
            this.holidayAdded.flexibilityStatus;
        }
      });
  ...
  deleteHoliday(id) {
    ...
    this.toastService.success(
      this.translate.instant('AnnualHoliday.Holiday deleted Successfully')
    );
    this.saveInlocalStorage();
    this.annualHolidayService.holidayList.next(this.holidayList);
  }
  ...
  getHolidayNameAndYear() {
    this.annualHolidayService.annualCalendarName.next(
      this.annualHolidayFormGrp.value.arabicSmester
    );
  ...
  filter() {
    ...
    if (this.annualHolidayFormGrp.value.status != '') {
      this.holidayList = this.holidayList.filter(
        (val) =>
  ...
  bindOldHoliday(holiday) {
    if (this.urlParameter) {
      this.annualHolidayFormGrp.patchValue({
        year: holiday.year,
      ...
      this.holidayList = this.holidayList.map((holiday, i) => {
        return {
          ...
          dateFrom: holiday.dateFrom,
          dateTo: holiday.dateTo,
          flexibilityStatus: this.holidayStatusList.find(
            (s) => s.value == holiday.flexibilityStatus
          ),
          schools: holiday.schools,
          createdDate: holiday.createdDate,
        };
