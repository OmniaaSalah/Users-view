...
export class ManualCertificatesComponent implements OnInit  {
  ...
  parentId = this.route.snapshot.paramMap.get('id');  // will be exist incase the page visited through parent children list
  studentGUID = this.route.snapshot.paramMap.get('id') || this.route.snapshot.paramMap.get('childId');
  studentId
...
submitOtherCertifiates(){
  this.onSubmit=true;
    let data = {
      ...
      this.certificateService.submitOtherCertifiatesBySpea(data).subscribe(result=>{
      this.onSubmit=false;
  ...
  checkDashboardHeader()
  {
      ...
      if(this.currentUserScope==UserScope.Employee)
      {
        ...
        this.componentHeaderData.breadCrump= [
          ...
          { label: this.translate.instant('dashboard.parents.sonDetails'),routerLink:`/student-management/all-parents/parent/${this.parentId}/child/${this.studentId}` }
