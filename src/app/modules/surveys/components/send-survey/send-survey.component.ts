...
export class SendSurveyComponent implements OnInit, OnDestroy {
  ...
  getParentList() {
    ...
    this.filtration.PageSize = 30;
    this.surveyService.getGuardians(this.filtration).subscribe(
      (res) => {
        if (res.data) {
  ...
  SendSurvey() {
    ...
    this.surveyService.sendSurvey(survey).subscribe(
      (response) => {
  ...
  getSurveyById() {
    ...
    this.surveyService
      .getSurveyById(Number(this.surveyId))
      .subscribe((response) => {
        this.editSurvey = response.result.result;

        this.surveyFormGrp.patchValue({
          disAppearanceDate: this.editSurvey.disApperanceDate
            ? new Date(this.editSurvey.disApperanceDate)
            : null,
          appearanceDate: this.editSurvey.apperanceDate
            ? new Date(this.editSurvey.apperanceDate)
            : null,
        });
  ...
  selectAllParents(value) {
    if (value.checked) {
      this.allSelectedGuardian = this.parent.list.map((parent) => {
        return parent.id;
      });
