...
export class EditNewSchoolyearComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.schoolYearFormGrp = fb.group({
      schoolYearArabicName: [
        '',
        [Validators.required, Validators.maxLength(32)],
      ],
      schoolYearEnglishName: [
        '',
        [Validators.required, Validators.maxLength(32)],
      ],
  ...
  ngOnInit(): void {
    ...
    this.schoolYearService
      .getAllSchoolYears(this.filtration)
      .subscribe((res) => {
        this.schoolYearsList = res.data;
      });
    this.schoolYearService.studentsList.subscribe((res) => {
      this.studentsList = res;
    });
    this.weekendsList = this.sharedService.weekDays;
    this.schoolYearService.getAnnualCalenders().subscribe((res) => {
      this.annualCalendersList = res;
    });
    this.schoolYearService.topStudantsListLength.subscribe((res) => {
      this.topStudantsListLength = res;
    });
    this.sharedService.getAllNationalities().subscribe((res) => {
      ...
      breadCrump: [
        {
          ...
          routerLink: '/educational-settings/school-year/school-years-list',
          routerLinkActiveOptions: { exact: true },
        },
        {
          label:
            this.urlParameter == 0 || this.urlParameter.toString() == ''
              ? this.translate.instant('breadcrumb.Add New School Year')
              : this.translate.instant('breadcrumb.School Year Details'),
          routerLink:
            this.urlParameter == 0 || this.urlParameter.toString() == ''
              ? '/educational-settings/school-year/new-school-year'
              : '/educational-settings/school-year/display-school-year/' +
                this.urlParameter,
        },
      ],
      mainTitle: {
  ...
  get schoolYearArabicName() {
    return this.schoolYearFormGrp.controls[
      'schoolYearArabicName'
    ] as FormControl;
  }
  ...
  get ageRequirementToRegisterFromInsideCountry() {
    return this.schoolYearFormGrp.controls[
      'ageRequirementToRegisterFromInsideCountry'
    ] as FormControl;
  }
  get ageRequirementToRegisterFromOutsideCountry() {
    return this.schoolYearFormGrp.controls[
      'ageRequirementToRegisterFromOutsideCountry'
    ] as FormControl;
  }
  ...
  getCurrentSchoolYear(urLParameter) {
    this.schoolYearService.getSchoolYearByID(urLParameter).subscribe((res) => {
      ...
      if (this.schoolYear) {
        this.schoolYearStatus = this.schoolYear.schoolYearStatus?.name.en;
        this.bindOldSchoolYear(this.schoolYear);
      }
      if (!this.urlParameter) {
        //to show action buttons in case we need to copy data fron current or finished year
  ...
  getCurriculumsInSchoolYear() {
    this.schoolYearService
      .getCurriculumsInSchoolYear(this.urlParameter)
      .subscribe((res) => {
  ...
  sendSchoolYear() {
    ...
    this.schoolYearService
      .editSchoolYearStatus(Number(this.urlParameter))
      .subscribe(
        (res) => {
          this.getCurrentSchoolYear(this.urlParameter);
  ...
  saveDraftYear(continueId) {
    this.schoolYearObj = {
      schoolYearName: {
        ar: this.schoolYearFormGrp.value.schoolYearArabicName,
        en: this.schoolYearFormGrp.value.schoolYearEnglishName,
      },
    ...
    if (this.urlParameter) {
      ...
      this.schoolYearService.editSchoolYear(this.schoolYearObj).subscribe(
        (res) => {
          if (res.statusCode != 'BadRequest') {
            ...
            if (continueId == 1) {
              ...
              this.router.navigate([
                '/educational-settings/school-year/school-years-list',
              ]);
            }
      ...
      this.schoolYearService.addDraftSchoolYear(this.schoolYearObj).subscribe(
        (res) => {
          if (res.statusCode != 'BadRequest') {
  ...
  selectTopStudents(schoolYearId, curriculumId, gradeId) {
    ...
    this.schoolYearService
      .getTopStudentsInSpecificGrade(schoolYearId, curriculumId, gradeId)
      .subscribe((res) => {
        res.forEach((element) => {
  ...
  saveTopStudent(curriculmId, gardeId) {
    this.schoolYearService
      .addTopStudentsToSpecificGrade({
        schoolYearId: this.urlParameter,
        curriculumId: curriculmId,
        gradeId: gardeId,
        students: this.topStudentIds,
      })
      .subscribe(
        (res) => {
          this.toastService.success(
  ...
  getStudentsFiltration() {
    ...
    if (this.nationalityId && this.perecentge) {
      ...
      this.studentsList = this.studentsList.filter(
        (val) => val.percentage == this.perecentge
      );
    } else if (this.nationalityId && !this.perecentge) {
      this.studentsList = this.studentsList.filter(
        (val) => val.nationality.id == this.nationalityId
      );
    } else {
      this.schoolYearService.studentsList.subscribe((res) => {
        this.studentsList = res;
  ...
  openRow(curriculumId) {
    ...
    this.schoolYearService
      .getClassesInCurriculumsInSchoolYear(this.urlParameter, curriculumId)
      .subscribe(
        (res) => {
          this.curriculumClassList.find(
            (c) => c.curriculumId == curriculumId
  ...
  saveCurriculumsIds() {
    ...
    this.schoolYearService
      .saveCurriculumsToSchoolYear(this.urlParameter, this.curriculumsIds)
      .subscribe(
        (res) => {
  ...
  checkSchoolYearStatus() {
    if (
      this.schoolYearStatus == SchoolYearEnum.Finished ||
      this.schoolYearStatus == SchoolYearEnum.Current ||
      this.urlParameter
    ) {
      this.step = 2;
    } else {
  ...
  openUpgradeStudentModel=false
  selectedYearToUpgradeTo

  upgradeStudents(yearToUpgradeTo){
    const currentYear = this.userService.schoolYearId

    this.schoolYearService.upgradeStudents(currentYear, yearToUpgradeTo).subscribe(res =>{
      this.toastService.success(this.translate.instant('toasterMessage.upgradeStudents'));
      this.openUpgradeStudentModel=false
    },(err)=>{
      this.toastService.error(this.translate.instant('toasterMessage.error'));
